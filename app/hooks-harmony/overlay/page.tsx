...
export default function HooksHarmonyOverlayPage() {
  ...
  useEffect(() => {
    ...
    QRCode.toDataURL(requestUrl, { margin: 1, color: { dark: "#0d1117", light: "#ffffff" } }).then(setQrDataUrl);
  }, [requestUrl]);
  ...
  return (
    <div style={{ ...hhTheme, width: "1920px", height: "1080px", position: "relative", background: "transparent", overflow: "hidden" }}>
      <video autoPlay muted loop playsInline style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover" }}>
        <source src="/hooks-harmony-bg.webm" type="video/webm" />
      </video>
      ...
      <div style={{ position: "absolute", inset: 0 }}>
        <div style={panelStyle("left")}>
          ...
          <div style={{ color: "#a8dbe2", fontSize: 20, marginTop: 6 }}>{state?.nowPlaying?.artist || ""}</div>
          ...
          <div style={{ height: 1, background: "rgba(0,195,218,.35)", margin: "18px 0" }} />
...
function panelStyle(side: "left" | "right"): React.CSSProperties {
  return {
    ...
    padding: 28,
    width: "320px",
    boxShadow: "0 0 20px rgba(0,195,218,.4)",
    textAlign: "center",
  } as React.CSSProperties;
}
...
const miniPillStyle: React.CSSProperties = {
  ...
  border: "1px solid rgba(0,195,218,.3)",
  borderRadius: 10,
  padding: "10px 14px",
  textAlign: "center",
};
