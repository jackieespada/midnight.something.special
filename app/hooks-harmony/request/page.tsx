...
// Hooks + Harmony brand palette, scoped to this page only via inline
// CSS custom properties on the root wrapper below. These override the
// same variable names Midnight Special's components use, so all the
// shared button/card/input styles just work with H+H's colors instead.
const hhTheme = {
  "--stage": "#0d1117",
  "--stage2": "#161c26",
  "--haze": "#692dad",
...
export default function HooksHarmonyRequestPage() {
  ...
  async function submit() {
    ...
    const res = await fetch("/api/hooks-harmony/request", {
      method: "POST",
  ...
  async function submitWithTip() {
    ...
    setTipLoading(true);
    try {
      const res = await fetch("/api/hooks-harmony/tip-checkout", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    <div style={{ ...hhTheme, maxWidth: 520, margin: "0 auto", padding: "24px 16px 48px", background: "var(--stage)", minHeight: "100vh" }}>
      <div
        style={{
      ...
      <h1 style={{ fontSize: 24, margin: "0 0 4px" }}>Add to the People's Playlist</h1>
      ...
      <div style={cardStyle}>
        ...
        <input style={inputStyle} value={song} onChange={(e) => setSong(e.target.value)} placeholder="e.g. Dancing Queen" />
        <label style={labelStyle}>Artist</label>
        <input style={inputStyle} value={artist} onChange={(e) => setArtist(e.target.value)} placeholder="e.g. ABBA" />
        ...
        <label style={labelStyle}>Dedication or story (optional, 200 characters)</label>
        <textarea
          style={{ ...inputStyle, minHeight: 70, resize: "vertical", fontFamily: "inherit" }}
          value={message}
          onChange={(e) => setMessage(e.target.value.slice(0, 200))}
          placeholder="Tell us why this song, or who it's for"
        />
        ...
        {showToast && (
          <div style={{ marginTop: 12, fontSize: 13, color: "var(--gold)" }}>
            Added to the playlist — watch for it on stream.
          </div>
        )}
      ...
      <div style={cardStyle}>
        <div style={{ fontSize: 10.5, color: "var(--ink-dim)", textTransform: "uppercase", letterSpacing: ".08em", marginBottom: 6 }}>
          Playlist right now ({queue.length}/25)
        </div>
        {queue.length === 0 ? (
...
const btnStyle: React.CSSProperties = {
  ...
  color: "#08181a",
  boxShadow: "0 6px 18px rgba(0,195,218,.35)",
};
